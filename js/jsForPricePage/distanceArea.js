$(document).ready(function () {
    $("#addMilesInput").on("click", function () {
        ...
        $("#calculateKmOfTrip").on("click", function () {
            ...
            function showRouteOnMap() {
                ...
                directionsService.route(request, function (response, status) {
                    if (status == google.maps.DirectionsStatus.OK) {
                        ...
                        var routeInKm = response.routes[0].legs[0].distance.value;
                        if (location.host == "") {
                            ...
                            webWorker.postMessage(routeInKm);
            ...
            $("#loadingPopUp").show();
            $("#myMusic").prop("currentTime", 4).trigger("play");
            setTimeout(showRouteOnMap, 4000);
        });
        initialize()
    });
    ...
    $("#currentLocation").change(function () {
        if ($(this).is(":checked")) {
            if (navigator.geolocation) {
                if (location.host == "") {
                    alert("you need to have a host in order to have a geo Location")
                } else {
    ...
    $("#xToGoBackPricePage").on("click", function () {
         $("#distensArea").hide();
    });
});
